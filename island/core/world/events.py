...
async def world_setup(redis):
    logger.debug(f"Fetching world server details. [ACCESS_KEY = {WORLD_ACCESS_KEY}]")
    async with db.transaction():
        world_meta_list = World.query.where(World.access_key == WORLD_ACCESS_KEY).gino.iterate()
        async for world_meta in world_meta_list:
            world_meta = WorldMeta.from_orm(world_meta)
            world = WorldBase(redis, 
...
async def world_unset(redis):
    logger.debug(f"Unset and stopping all world servers. [ACCESS_KEY = {WORLD_ACCESS_KEY}]")
