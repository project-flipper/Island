...
async def handle_room_join(ws: WebSocket, packet: Packet[RoomJoinData], user: Annotated[UserTable, Depends(get_current_user)]):
    await send_packet(ws, 'room:join', RoomJoinResponse(room_id=packet.d.room_id, players=[Player(user=await User.from_table(user), x=packet.d.x, y=packet.d.y, action=DEFAULT_ACTION)]))

@packet_handlers.register('room:spawn')
async def handle_room_spawn(ws: WebSocket, packet: Packet, user: Annotated[UserTable, Depends(get_current_user)]):
